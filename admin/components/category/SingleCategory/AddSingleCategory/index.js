import React, { useEffect, useState } from "react";
import * as Yup from "yup";
import { db, timestamp } from "@/app/utils/firebase";

import { notifications } from "@mantine/notifications";
import SingleCategoryForm from "./SingleCategoryForm";
import { AppForm } from "@/admin/components/shared/Form";
import FormFooter from "@/admin/components/shared/FormFooter";
import FormHeader from "@/admin/components/shared/FormHeader";
import { uuid } from "@/admin/utils/helpers";

const validationSchema = Yup.object().shape({
  parent_category_title: Yup.string().required().label("Parent Category Title"),
  child_category_title: Yup.string().required().label("Child Category Title"),
  child_category_path: Yup.string().required().label("Child Category Path"),
  parent_category_uid: Yup.string().label("Select type"),
});

const AddSingleCategory = ({ onClick, item }) => {
  const [loading, setLoading] = useState(false);
  const uID = uuid();

  // place childCategory handler on submit
  const placeChildCategory = async (values) => {
    setLoading(true);
    const category_uid = values?.parent_category_uid;
    await placeChildCategoryHandler(values, category_uid);

    setLoading(false);
    notifications.show({
      title: "Success",
      message: `Category ${values.category_title} added seccessfully`,
      color: "blue",
      autoClose: 5000,
    });
    //for close the tab
    onClick();
  };
  // save child Category details on firebase database
  const placeChildCategoryHandler = async (values, category_uid) => {
    await db
      .collection("category")
      .doc("childCategory")
      .collection(category_uid)
      .doc(uID)
      .set({
        child_category_path: values.child_category_path,
        parent_category_title: values.parent_category_title,
        parent_category_uid: values.parent_category_uid,
        name: values.child_category_title,
        id: values.child_category_title,
        timestamp,
        isPublished: false,
        uid: uID,
      });
  };
  return (
    <main>
      <div>
        <AppForm
          initialValues={{
            parent_category_title: (item && item?.category_title) || "",
            child_category_title: "",
            child_category_path: (item && item?.category_path) || "",
            parent_category_uid: (item && item?.category_uid) || "",
          }}
          onSubmit={placeChildCategory}
          validationSchema={validationSchema}
        >
          <div className="h-screen relative">
            <div className="w-full">
              <FormHeader
                onClick={onClick}
                title="Add Child Category"
                sub_title={
                  "Add your child category and necessary information from here"
                }
              />
            </div>

            <div className="w-full h-[75%] md:h-[80%] overflow-y-scroll py-3 px-6 md:px-4 mb-4">
              <SingleCategoryForm uid={"101"} />
            </div>

            <div className="fixed bottom-0 right-0 w-full bg-gray-50">
              <FormFooter
                title={"Submit"}
                onClick={onClick}
                loading={loading}
                acceptBtn={placeChildCategory}
              />
            </div>
          </div>
        </AppForm>
      </div>
    </main>
  );
};

export default AddSingleCategory;
